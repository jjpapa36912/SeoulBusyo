...
// MARK: - Const & Utils
private let CITY_CODE = 31010 // 수원
//private let CITY_CODE = 31020 // 성남
//private let CITY_CODE = 31030 // 의정부시 최후에 하자
//private let CITY_CODE = 31040 // 안양
//private let CITY_CODE = 31050 // 부천
//private let CITY_CODE = 31060 // 광명
//private let CITY_CODE = 31070 // 평택
//private let CITY_CODE = 31080 // 동두천 최후에 하자
//private let CITY_CODE = 31090 // 안산
//private let CITY_CODE = 31100 // 고양시
//private let CITY_CODE = 31110 // 과천 31120
//private let CITY_CODE = 31120 // 구리
...
//    private var seoulServiceKeyRaw: String = "FVUZJTrP1WLAsFAKcXy8lh2Qy1DWNw5Ul2%2BvSY01E3cUJlO%2F9P%2BCodODXPIyzppQCPswXvc1WeblEAh6X41ClA%3D%3D"
    ...
    private func maskKeyPreview(_ key: String) -> String {
        ...
        let tail = key.suffix(3)
        return "\(head)•••\(tail)"
    }

    // 일부 서버는 '+'를 공백으로 오해하니 '%2B'로 강제 이스케이프한 변형도 시도
    private func seoulKeyCandidates(from raw: String) -> [String] {
        // 1) 발급 그대로
        let raw1 = raw
        // 2) '+' -> '%2B' 강제
        let rawPlusEscaped = raw.replacingOccurrences(of: "+", with: "%2B")
        // 3) percent-decoded(혹시 저장 중 이중인코딩됐다면 복원)
        let decoded = raw.removingPercentEncoding ?? raw
        // 4) decoded + plus-escape
        let decodedPlusEscaped = decoded.replacingOccurrences(of: "+", with: "%2B")
        // 5) URLQueryAllowed로 1회 인코딩
        let encodedOnce = raw.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? raw
        // 6) encodedOnce + plus-escape(방어)
        let encodedOncePlusEscaped = encodedOnce.replacingOccurrences(of: "+", with: "%2B")

        // 중복 제거 + 순서 유지
        var seen = Set<String>(); var out: [String] = []
        for k in [raw1, rawPlusEscaped, decoded, decodedPlusEscaped, encodedOnce, encodedOncePlusEscaped] {
    ...
    func fetchRoutePath(cityCode: Int, routeId: String) async throws -> [CLLocationCoordinate2D] {
        ...
        // 필드: gpsX / gpsY
        let out = arr.compactMap { d -> CLLocationCoordinate2D? in
            guard let la = toDouble(d["gpsy"]) ?? toDouble(d["gpsy"]),
                  let lo = toDouble(d["gpsx"]) ?? toDouble(d["gpsx"]) else { return nil }
    ...
    func fetchStopsByRoute(cityCode: Int, routeId: String) async throws -> [BusStop] {
        Telemetry.shared.mark(.fetchArrivalsBegin, tags:["fn":"fetchStopsByRoute(Seoul)"])

        guard var comps = URLComponents(string: "http://ws.bus.go.kr/api/rest/busRouteInfo/getStaionByRtid") else {
            throw APIError.invalidURL
        }
        comps.queryItems = [
            .init(name: "serviceKey", value: serviceKeyRaw.encodedForServiceKey),
        ...
        let (data, _) = try await send("RouteStops(Seoul)", url: url)
        let arr = try parseXMLItems(data)

        // 대표 필드: arsId(정류장번호), stId(내부ID), stationNm, gpsX/gpsY
        let out: [BusStop] = arr.compactMap { d in
            let id = d["arsid"] ?? d["arsId"] ?? d["stId"]       // 도착API 호환을 위해 arsId 선호
            let name = d["stationnm"] ?? d["stationNm"] ?? d["stNm"]
            let la = toDouble(d["gpsy"]) ?? toDouble(d["gpsy"]) ?? toDouble(d["wgs84Lat"])
            let lo = toDouble(d["gpsx"]) ?? toDouble(d["gpsx"]) ?? toDouble(d["wgs84Lon"])
            guard let id, let name, let la, let lo else { return nil }
            return BusStop(id: id, name: name, lat: la, lon: lo, cityCode: 0)
        }
    ...
    // BusAPI 내부 교체 (서울시: 근처 정류장 조회)
    // BusAPI 내부 교체: 서울시 근처 정류장 조회 (TM → WGS84 폴백, JSON 파서 보강)
    func fetchStops(lat: Double, lon: Double) async throws -> [BusStop] {
        Telemetry.shared.mark(.fetchStopsBegin, tags: ["fn": "fetchStops(Seoul)"])

        @inline(__always)
        func peek(_ data: Data, limit: Int = 260) -> String {
            let s = String(data: data, encoding: .utf8) ?? "<non-utf8 \(data.count)b>"
            return String(s.prefix(limit)).replacingOccurrences(of: "\n", with: " ")
        }

        @inline(__always)
        func parseHeader(_ data: Data) -> (cd: String?, msg: String?, cnt: Int?) {
            guard let s = String(data: data, encoding: .utf8) else { return (nil, nil, nil) }
            func grabStr(_ key: String) -> String? {
                if let r = try? NSRegularExpression(pattern: "\"\(key)\"\\s*:\\s*\"(.*?)\""),
                   let m = r.firstMatch(in: s, range: NSRange(s.startIndex..., in: s)),
                   let rr = Range(m.range(at: 1), in: s) { return String(s[rr]) }
                if let r = try? NSRegularExpression(pattern: "\"\(key)\"\\s*:\\s*(\\d+)"),
                   let m = r.firstMatch(in: s, range: NSRange(s.startIndex..., in: s)),
                   let rr = Range(m.range(at: 1), in: s) { return String(s[rr]) }
                if let r = try? NSRegularExpression(pattern: "<\(key)>(.*?)</\(key)>"),
                   let m = r.firstMatch(in: s, range: NSRange(s.startIndex..., in: s)),
                   let rr = Range(m.range(at: 1), in: s) { return String(s[rr]) }
                return nil
            }
            let cd  = grabStr("headerCd")
            let msg = grabStr("headerMsg")
            let cnt = Int(grabStr("itemCount") ?? "")
            return (cd, msg, cnt)
        }
        ...
        func parseStopsJSON(_ data: Data, tag: String) -> [BusStop] {
            ...
            let root: [String: Any] = anyRoot ?? [:]
            let container = (root["ServiceResult"] as? [String: Any]) ?? root
            let body = container["msgBody"] as? [String: Any]

            if anyRoot == nil {
                print("⚠️ [\(tag)] JSON decode 실패")
                return []
            }
            if body == nil {
                let keys = root.keys.sorted().joined(separator: ",")
                print("⚠️ [\(tag)] msgBody 없음 — rootKeys=[\(keys)]")
                return []
            }
            ...
            func normalizeList(_ any: Any?) -> [[String: Any]] {
                ...
                if let s = any as? String, s.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty { return [] }
                return []
            }
            ...
            let hdrCd  = header?["headerCd"] ?? "nil"
            let hdrMsg = header?["headerMsg"] ?? "nil"
            print("📦 [\(tag)] headerCd=\(hdrCd) headerMsg=\(hdrMsg) headerItemCount=\(hdrCnt) listCount=\(list.count)")

            func dstr(_ d: [String: Any], _ k: String) -> String? {
                if let v = d[k] as? String { return v }
                if let v = d[k] as? NSNumber { return v.stringValue }
                return nil
            }
            func ddouble(_ d: [String: Any], _ k: String) -> Double? {
                if let s = d[k] as? String, let x = Double(s) { return x }
        ...
        do {
            ...
            print("🧾 Stops(Seoul:Pos:TM:800) headerCd=\(hdr.cd ?? "?") msg=\(hdr.msg ?? "?") itemCount=\(hdr.cnt ?? -1)")
            let stops = parseStopsJSON(data, tag: "Stops(Seoul:Pos:TM:800)")
            Telemetry.shared.mark(.fetchStopsEnd, tags:["src":"Seoul:TM:800"], fields:["count": Double(stops.count)])
            if !stops.isEmpty { return stops }
            print("⚠️ TM:800 결과 0건 — posX/posY로 폴백")
        } catch {
    ...
    func fetchArrivalsDetailed(cityCode: Int, nodeId: String) async throws -> [ArrivalInfo] {
        ...
        // 공용 doGET/peek 유틸이 없다면, 아래 두 개를 이 메서드 맨 위에 로컬함수로 넣어도 OK
        @inline(__always)
        func peek(_ data: Data, limit: Int = 260) -> String {
            let s = String(data: data, encoding: .utf8) ?? "<non-utf8 \(data.count)b>"
            return String(s.prefix(limit)).replacingOccurrences(of: "\n", with: " ")
        ...
        let url = "\(base)?serviceKey=\(seoulServiceKeyRaw)&arsId=\(nodeId)&resultType=json"

        let data = try await doGET(url, name: "Arrivals(Seoul:ByUid)")

        // ── JSON 파싱: ServiceResult 감싸기/단일/배열/빈값 모두 대응 ──
        let anyRoot = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any] ?? [:]
        let container = (anyRoot["ServiceResult"] as? [String: Any]) ?? anyRoot
        let header = container["msgHeader"] as? [String: Any]
        let body = container["msgBody"] as? [String: Any]
        ...
        let list = normalizeList(body?["itemList"])
        let code = (header?["headerCd"] as? String) ?? ""
        let msg  = (header?["headerMsg"] as? String) ?? ""
        print("🧾 headerCd=\(code) headerMsg=\(msg) listCount=\(list.count)")
        ...
        // d[key] → Int?
        func dint(_ d: [String: Any], _ k: String) -> Int? {
            if let s = d[k] as? String, let x = Int(s) { return x }
        ...
        var out: [ArrivalInfo] = []
        for it in list {
            guard let rid = dstr(it, "busRouteId"),
                  let rno = dstr(it, "rtNm") else { continue }

            // 우선순위: traTime1/2(초) > arrmsg(문자) 교차 사용
            let sec1 = dint(it, "traTime1")
            let sec2 = dint(it, "traTime2")
            let msg1 = dstr(it, "arrmsg1")?.trimmingCharacters(in: .whitespaces) ?? ""
            let msg2 = dstr(it, "arrmsg2")?.trimmingCharacters(in: .whitespaces) ?? ""
            let dest = dstr(it, "adirection") ?? dstr(it, "stEnd") ?? dstr(it, "direction")

            // “운행종료/출발대기/곧도착” 등의 텍스트가 들어오므로 보정
            func etaMinFrom(sec: Int?, msg: String) -> Int? {
                if let s = sec, s > 0 { return max(0, s/60) }
                // 보조: “곧 도착”류 메시지 → 1~2분 가정 (필요시 조정)
                if msg.contains("곧") || msg.contains("잠시후") { return 1 }
                // 운행종료/출발대기/대기 등은 스킵
                if msg.contains("운행종료") || msg.contains("출발대기") { return nil }
                return nil
            }

            if let m1 = etaMinFrom(sec: sec1, msg: msg1) {
                out.append(.init(routeId: rid, routeNo: rno, etaMinutes: m1, destination: dest))
            }
            if let m2 = etaMinFrom(sec: sec2, msg: msg2) {
                out.append(.init(routeId: rid, routeNo: rno, etaMinutes: m2, destination: dest))
            }
        }
        ...
        Telemetry.shared.mark(.fetchArrivalsEnd,
                              tags:["src":"Seoul", "arsId": nodeId, "fmt":"json", "code": code],
                              fields:["count": Double(out.count)])
        return out
    }
...
//    private var routeNoByRouteId: [String:String] = [:]
    ...
    func fetchBusLocations(cityCode: Int, routeId: String) async throws -> [BusLive] {
        ...
        // 0) 노선번호 캐시 확보: 한 번도 못 본 routeId라면 routeAll로 rtNm 받아서 캐시에 저장
        var commonRouteNo: String? = routeNoByRouteId[routeId]
        if commonRouteNo == nil {
            let baseR = "http://ws.bus.go.kr/api/rest/arrive/getArrInfoByRouteAll"
            let urlR  = "\(baseR)?serviceKey=\(seoulServiceKeyRaw)&busRouteId=\(routeId)&resultType=json"
            do {
                let dataR = try await doGET(urlR, name: "RouteAll(Seoul:\(routeId))")
                let rootR = (try? JSONSerialization.jsonObject(with: dataR)) as? [String: Any] ?? [:]
                let contR = (rootR["ServiceResult"] as? [String: Any]) ?? rootR
                let bodyR = contR["msgBody"] as? [String: Any]
                let listR = normalizeList(bodyR?["itemList"])
                // rtNm 첫 번째라도 있으면 채택
                if let first = listR.first, let nm = dstr(first, "rtNm"), !nm.isEmpty {
                    commonRouteNo = nm
                    routeNoByRouteId[routeId] = nm
                }
            } catch {
                // 실패해도 진행 (버스 위치엔 영향 없음)
                print("⚠️ RouteAll fetch failed for \(routeId): \(error.localizedDescription)")
            }
        }

        // 1) 버스 위치 호출(JSON 강제)
        let base = "http://ws.bus.go.kr/api/rest/buspos/getBusPosByRtid"
        let url  = "\(base)?serviceKey=\(seoulServiceKeyRaw)&busRouteId=\(routeId)&resultType=json"
        let data = try await doGET(url, name: "BusLoc(Seoul:ByRtid)")
        ...
        let msg    = (header?["headerMsg"] as? String) ?? ""
        let list   = normalizeList(body?["itemList"])
        print("🧾 BusLoc headerCd=\(code) headerMsg=\(msg) listCount=\(list.count) routeId=\(routeId)")
        ...
        for it in list {
            // 2-1) 차량 ID
            let veh = dstr(it, "vehId") ?? dstr(it, "plainNo") ?? dstr(it, "vehid") ?? dstr(it, "plainno")

            // 2-2) 노선번호: 아이템의 rtNm → 캐시 → routeId(최후의 보루)
            var routeNo: String? = nil
            if routeNo == nil, let v = dstr(it, "rtNm"), !v.isEmpty { routeNo = v }
            if routeNo == nil, let v = dstr(it, "rtnm"), !v.isEmpty { routeNo = v }
            if routeNo == nil { routeNo = commonRouteNo }
            if routeNo == nil { routeNo = routeNoByRouteId[routeId] }
            if routeNo == nil { routeNo = routeId } // (보이는 ? 방지용 최후 fallback)

            // 2-3) 좌표(WGS84)
            let lon = ddouble(it, "gpsX") ?? ddouble(it, "wgs84Lon")
            let lat = ddouble(it, "gpsY") ?? ddouble(it, "wgs84Lat")

            // 2-4) 다음 정류장명
            let nextName = dstr(it, "stationNm") ?? dstr(it, "nodeNm") ?? dstr(it, "nextStn") ?? dstr(it, "nxtStn")

            guard let veh, let lat, let lon else { skippedNoCoord += 1; continue }
            if !( (34.0...39.5).contains(lat) && (125.0...130.0).contains(lon) ) {
                skippedNoCoord += 1
                print("⚠️ skip invalid WGS84 lat/lon veh=\(veh) lat=\(lat) lon=\(lon)")
                continue
            }

            out.append(BusLive(id: veh,
                               routeNo: routeNo ?? routeId,
                               lat: lat, lon: lon,
                               etaMinutes: nil,
        ...
        // 3) 최종 로그 + 텔레메트리
        if let nm = commonRouteNo { routeNoByRouteId[routeId] = nm } // 보강
        Telemetry.shared.mark(.fetchBusLocEnd,
                              tags:["src":"Seoul", "code":code, "routeId": routeId, "routeNo": routeNoByRouteId[routeId] ?? (commonRouteNo ?? "-")],
                              fields:["count": Double(out.count), "skippedNoCoord": Double(skippedNoCoord)])
        if let first = out.first {
            print("🧭 firstBus veh=\(first.id) rno=\(first.routeNo) lat=\(first.lat) lon=\(first.lon) next=\(first.nextStopName ?? "-")")
        }
        return out
    }
...
final class BusStopAnnotation: NSObject, MKAnnotation {
    ...
    // ✅ MapKit KVO(값 변화 감지)를 위해 @objc dynamic var로 선언
    @objc dynamic var coordinate: CLLocationCoordinate2D
    
    // ✅ title을 저장형 변수로 변경하여 상시 노출 및 수정 가능하게 함
    @objc dynamic var title: String?
    
    // ✅ subtitle을 추가하여 "X개 노선 도착 예정" 등의 정보를 직접 대입할 수 있게 함
    @objc dynamic var subtitle: String?

    init(_ s: BusStop) {
        self.stop = s
        self.coordinate = .init(latitude: s.lat, longitude: s.lon)
        self.title = s.name // 초기값으로 정류장 이름 설정
        super.init()
    }
}
...
final class BusAnnotation: NSObject, MKAnnotation {
    ...
    // ✅ MapKit KVO 및 외부 대입을 위해 @objc dynamic var로 선언
    @objc dynamic var coordinate: CLLocationCoordinate2D
    @objc dynamic var title: String?
    @objc dynamic var subtitle: String?
    ...
    func update(to b: BusLive) {
        ...
        self.subtitle = Self.makeSubtitle(eta: b.etaMinutes, next: b.nextStopName)
...
final class ClusterView: MKAnnotationView {
    ...
    override init(annotation: MKAnnotation?, reuseIdentifier: String?) {
        ...
        // 1. 크기를 살짝 키우고 원형 유지 (34 -> 42)
        frame = CGRect(x: 0, y: 0, width: 42, height: 42)
        layer.cornerRadius = 21
        
        // 2. 단일 버스 아이콘과 확실히 구분되도록 굵은 흰색 테두리 추가
        layer.borderWidth = 3
        layer.borderColor = UIColor.white.cgColor
        
        // 3. 그림자를 넣어 지도로부터 띄워 보이게 함 (입체감)
        ...
        // 4. 라벨 설정 (아주 굵은 폰트 사용)
        countLabel.font = .systemFont(ofSize: 13, weight: .black)
    ...
    override func prepareForDisplay() {
        ...
        if let cluster = annotation as? MKClusterAnnotation {
            ...
            if isStopCluster {
                ...
                // ✅ 버스 클러스터: 🚌 기호 추가
                countLabel.text = "🚌 \(count)"
                // 일반 버스(밝은 파랑)와 구분되도록 조금 더 진한 남색(Navy) 계열 사용
                backgroundColor = UIColor(red: 0.0, green: 0.25, blue: 0.55, alpha: 0.95)
            }
            
            // 클러스터가 일반 마커보다 뒤로 가거나 가려지지 않게 우선순위 조절
...
final class MapVM: ObservableObject {
    // MapVM 클래스 내부에 추가할 변수들
    @Published var roiRegion: MKCoordinateRegion = MKCoordinateRegion(
        center: .init(latitude: 37.5665, longitude: 126.9780),
        span: .init(latitudeDelta: 0.005, longitudeDelta: 0.005)
    )
    @Published var busCountInROI: Int = 0
    @Published var stopCountInROI: Int = 0

    // MapVM 내부에 추가할 메서드
    func updateROILogic(mapView: MKMapView) {
        let screenWidth = mapView.bounds.width
        let screenHeight = mapView.bounds.height
        let roiRect = CGRect(x: (screenWidth - 180) / 2, y: (screenHeight - 180) / 2, width: 180, height: 180)
        let region = mapView.convert(roiRect, toRegionFrom: mapView)
        
        DispatchQueue.main.async {
            self.roiRegion = region
            let visibleAnnos = mapView.annotations(in: mapView.visibleMapRect).compactMap { $0 as? MKAnnotation }
            let inROI = visibleAnnos.filter { anno in
                abs(region.center.latitude - anno.coordinate.latitude) < region.span.latitudeDelta / 2 &&
                abs(region.center.longitude - anno.coordinate.longitude) < region.span.longitudeDelta / 2
            }
            self.busCountInROI = inROI.filter { $0 is BusAnnotation }.count
            self.stopCountInROI = inROI.filter { $0 is BusStopAnnotation }.count
        }
    }
    
    func canonicalRouteNo(_ raw: String) -> String {
        let t = raw.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
        
        // 1. 하이픈(-)이나 언더바(_)가 있으면 그 앞부분만 가져옴
        let separators = CharacterSet(charactersIn: "-_")
        let parts = t.components(separatedBy: separators)
        var head = parts.first ?? t
        
        // 2. 숫자로 시작하는데 뒤에 문자가 붙어 있는 경우 숫자만 추출 (예: 102HILM -> 102)
        if let firstLetterIdx = head.firstIndex(where: { $0.isLetter }) {
            if firstLetterIdx != head.startIndex {
                head = String(head[..<firstLetterIdx])
            }
        }
        return head
    }
    ...
    // MapVM 내부 아무 곳(기존 코드 보존) — 보조 유틸 추가
    private func _normRouteNo(_ s: String) -> String {
        // 공백/개행 제거 + 전각숫자 → 반각 + 대문자
        let trimmed = s.trimmingCharacters(in: .whitespacesAndNewlines)
        let mapped = trimmed.applyingTransform(.fullwidthToHalfwidth, reverse: false) ?? trimmed
...
//       }
    ...
    private func resolveRouteId(for routeNo: String) -> String? {
        ...
        // 🔽 추가: 정규화 키로도 조회
        let k = _normRouteNo(routeNo)
        if let id = routeIdByRouteNo[k] { return id }
        if let id = lastKnownRouteIdByRouteNo[k] { return id }

        if let id = latestTopArrivals.first(where: { _normRouteNo($0.routeNo) == k })?.routeId {
            routeIdByRouteNo[routeNo] = id
            routeIdByRouteNo[k] = id                // 정규화 키도 캐시
            lastKnownRouteIdByRouteNo[routeNo] = id
            lastKnownRouteIdByRouteNo[k] = id
            return id
        }
        return nil
    }
    ...
    private func mergeAndFilter(_ incoming: [BusLive], snap: RouteSnapshot) -> [BusLive] {
        ...
        for b in incoming {
            ...
            // rid가 없으면 보정 시도 없이 그대로 통과 (rid는 else 블록에서 쓸 수 없음)
            guard let rid = resolveRouteId(for: b.routeNo) else {
                out.append(b)
                continue
            }

            // ── routeNo 보정: "?" 이면 캐시/최근 ETA에서 복원
            let correctedRouteNo: String = {
                guard b.routeNo == "?" else { return b.routeNo }
                if let known = routeNoByRouteId[rid] {
                    return known
                }
                if let known2 = latestTopArrivals.first(where: { $0.routeId == rid })?.routeNo {
                    routeNoByRouteId[rid] = known2
                    return known2
                }
                return b.routeNo
            }()
            ...
            if followBusId == bus.id {
                ...
                setFutureRouteByStops(meta: meta, from: prj, nextIdx: nextIdx,
                                      maxAheadStops: 7, includeTerminal: false)
            }
    ...
    private func computeTopArrivals(
        ...
        for a in allArrivals {
            // ✅ 0분/음수(=사실상 없음) 스킵
            guard a.etaMinutes > 0 else { continue }
...
extension BusAPI {
    ...
    // BusAPI 내부
    /// 서울시: 노선번호 검색으로 routeId 목록
    func fetchRouteIdsByRouteNo(cityCode: Int, routeNo: String) async throws -> [String] {
        guard var comps = URLComponents(string: "http://ws.bus.go.kr/api/rest/busRouteInfo/getBusRouteList") else {
            throw APIError.invalidURL
        }
        comps.queryItems = [
            .init(name: "serviceKey", value: serviceKeyRaw.encodedForServiceKey),
        ...
        let (data, http) = try await send("RouteNoList(Seoul)", url: url)
        if let s = String(data: data, encoding: .utf8) {
            print("🔎 RouteNoList(Seoul) raw(\(http.statusCode)): \(min(240, s.count)) chars → \(s.prefix(240))")
        }
        let arr = try parseXMLItems(data)
        // 필드: busRouteId, busRouteNm, etc.
        let ids = arr.compactMap { $0["busrouteid"] ?? $0["busRouteId"] }
        let sample = ids.prefix(5)
        print("🔎 RouteNoList(Seoul) for '\(routeNo)' → total=\(ids.count), sample=\(sample)")
        return ids
    }
...
enum BusProvider { case seoul }
private let provider: BusProvider = .seoul
...
struct ClusteredMapView: UIViewRepresentable {
    ...
    // ✅ 모드 추가: explore(하단 탐색), detail(상단 확대)
    var mode: MapMode = .explore
    enum MapMode { case explore, detail }
    ...
    func makeUIView(context: Context) -> MKMapView {
        ...
        // ✅ 서울/수원 중심 초기화 (제공해주신 좌표)
        let startCenter = CLLocationCoordinate2D(latitude: 37.5665, longitude: 126.9780)
        let startRegion = MKCoordinateRegion(center: startCenter, span: .init(latitudeDelta: 0.01, longitudeDelta: 0.01))
        ...
        // 2. 상단 상세용 리얼 디자인 (2열 종대 및 고시인성)
        map.register(RealisticStopView.self, forAnnotationViewWithReuseIdentifier: "stop-detail")
        map.register(RealisticBusView.self, forAnnotationViewWithReuseIdentifier: "bus-detail")
    ...
    func updateUIView(_ uiView: MKMapView, context: Context) {
        // 1) 내 위치 버튼 처리 (Explore 모드에서만 작동)
        if mode == .explore && recenterRequest {
            defer { DispatchQueue.main.async { self.recenterRequest = false } }
            if let loc = uiView.userLocation.location?.coordinate {
                context.coordinator.centerOn(loc, mapView: uiView, animated: true)
            }
        }

        // 2) 상단 상세 맵 동기화 및 제스처 제한
        if mode == .detail {
            uiView.setRegion(vm.roiRegion, animated: true)
        ...
        context.coordinator.applyAnnotationDiff(
            on: uiView,
            stopsToAdd: vm.stops.filter { s in !currentStops.contains(where: {$0.stop.id == s.id}) }.map { BusStopAnnotation($0) },
            stopsToRemove: currentStops.filter { s in !vm.stops.contains(where: {$0.id == s.stop.id}) },
            busesToAdd: vm.buses.filter { b in !currentBuses.contains(where: {$0.id == b.id}) }.map { BusAnnotation(bus: $0) },
            busesToRemove: currentBuses.filter { b in !vm.buses.contains(where: {$0.id == b.id}) && b.id != vm.followBusId },
            busUpdates: vm.buses.compactMap { b in if let anno = currentBuses.first(where: {$0.id == b.id}) { return (anno, b) }; return nil }
        )

        // 4) 후처리 (탐색 모드일 때만 자동 팔로우 등 실행)
        if mode == .explore {
            ...
            context.coordinator.updateTrailOverlay(uiView)
            context.coordinator.updateFutureRouteOverlay(uiView)
    ...
    final class Coord: NSObject, MKMapViewDelegate {
        ...
        private var isTweakingFollowAppearance = false

        init(_ p: ClusteredMapView) { parent = p }
        // ClusteredMapView.Coord 클래스 내부에 아래 메서드를 반드시 포함해야 합니다.

        func updateFollowTints(_ mapView: MKMapView) {
            // 1. 현재 맵에 있는 모든 어노테이션 중 버스만 골라냅니다.
            let busAnnos = mapView.annotations.compactMap { $0 as? BusAnnotation }
            let followedId = parent.vm.followBusId
            
            UIView.performWithoutAnimation {
                CATransaction.begin()
                CATransaction.setDisableActions(true)
                
                for anno in busAnnos {
                    guard let view = mapView.view(for: anno) as? BusMarkerView else { continue }
                    
                    let isFollowed = (anno.id == followedId)
                    
                    // ✅ 팔로우 중인 버스는 클러스터링(합쳐짐)에서 제외
                    view.clusteringIdentifier = isFollowed ? "bus-followed-\(anno.id)" : "bus"
                    
                    // ✅ 상태에 맞는 색상 적용
                    view.configureTint(isFollowed: isFollowed)
        ...
        // ✅ [ADD] 상단 상세 맵용 2열 종대 정렬 메서드
        private func arrangeBusesInGrid(_ mapView: MKMapView) {
            guard parent.mode == .detail else { return }
            let busAnnos = mapView.annotations.compactMap { $0 as? BusAnnotation }
            var groups: [[BusAnnotation]] = []
            let threshold: CLLocationDistance = 35.0

            for bus in busAnnos {
                if let i = groups.firstIndex(where: { CLLocation(latitude: $0[0].coordinate.latitude, longitude: $0[0].coordinate.longitude).distance(from: CLLocation(latitude: bus.coordinate.latitude, longitude: bus.coordinate.longitude)) < threshold }) { groups[i].append(bus) }
                else { groups.append([bus]) }
            }
            for group in groups {
                let sorted = group.sorted { $0.routeNo < $1.routeNo }
                for (i, bus) in sorted.enumerated() {
                    guard let v = mapView.view(for: bus) else { continue }
                    let col = i % 2, row = i / 2
                    UIView.animate(withDuration: 0.3) {
                        v.centerOffset = CGPoint(x: (CGFloat(col) - 0.5) * 85.0, y: -35.0 - (CGFloat(row) * 75.0))
                    }
                }
            }
        }

        // ✅ [REPLACE] 뷰 팩토리 (모드별 분기 및 Non-optional 처리)
        func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
            if annotation is MKUserLocation { return nil }

            if let s = annotation as? BusStopAnnotation {
                if parent.mode == .detail {
                    let v = mapView.dequeueReusableAnnotationView(withIdentifier: "stop-detail", for: s) as! RealisticStopView
                    ...
                    v.markerTintColor = isAlarmed ? .systemYellow : (parent.vm.isStopSelected(s.stop.id) ? .systemOrange : .systemRed)
                    return v
                }
            } else if let b = annotation as? BusAnnotation {
                if parent.mode == .detail {
                    let v = mapView.dequeueReusableAnnotationView(withIdentifier: "bus-detail", for: b) as! RealisticBusView
                    v.canShowCallout = true
                    v.clusteringIdentifier = nil // 그리드 정렬을 위해 해제
                    ...
                    v.clusteringIdentifier = isFollowed ? "bus-followed-\(b.id)" : "bus"
        ...
        // ✅ [REPLACE] 클릭 이벤트 (Subtitle 수정 가능 버전)
        func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
            if let bus = view.annotation as? BusAnnotation {
                ...
                let next = bus.nextStopName ?? "추정 중..."
                bus.subtitle = "다음: \(next)"
                
                if parent.mode == .detail { mapView.deselectAnnotation(bus, animated: true) }
                return
            }
            ...
            if let stop = view.annotation as? BusStopAnnotation {
                Task {
                    ...
                    let count = self.parent.vm.focusStopETAs.count
                    await MainActor.run { stop.subtitle = "\(count)개 노선 도착 예정" }
                    if parent.mode == .detail { mapView.deselectAnnotation(stop, animated: true) }
                }
                return
            }
        }

        // ✅ [REPLACE] 영역 변경 시 ROI 및 그리드 업데이트
        func mapView(_ mapView: MKMapView, regionDidChangeAnimated animated: Bool) {
            deb.call(after: 0.18) { [weak self, weak mapView] in
                guard let self, let mapView else { return }
                if self.parent.mode == .explore {
                    self.parent.vm.updateROILogic(mapView: mapView)
                } else {
                    self.arrangeBusesInGrid(mapView)
                }
                self.parent.vm.onRegionCommitted(mapView.region)
            }
        }

        // ✅ [REPLACE] Diff 로직 통합 (배치 후 모드별 정렬 실행)
        ...
        func follow(_ anno: BusAnnotation, on mapView: MKMapView) {
            ...
            if CLLocation(latitude: center.latitude, longitude: center.longitude).distance(from: CLLocation(latitude: anno.coordinate.latitude, longitude: anno.coordinate.longitude)) > 30 {
        ...
        func updateTrailOverlay(_ mapView: MKMapView) {
            ...
            if let line = parent.vm.trail.polyline() { mapView.addOverlay(line) }
        }

        func updateFutureRouteOverlay(_ mapView: MKMapView) {
            let olds = mapView.overlays.filter { ($0 as? MKPolyline)?.title == "futureRoute" }
            mapView.removeOverlays(olds)
            if parent.vm.futureRouteCoords.count >= 2 {
                let line = MKPolyline(coordinates: parent.vm.futureRouteCoords, count: parent.vm.futureRouteCoords.count)
        ...
        func recolorStops(_ mapView: MKMapView) {
            let annos = mapView.annotations.compactMap { $0 as? BusStopAnnotation }
            for s in annos {
                guard let v = mapView.view(for: s) as? MKMarkerAnnotationView else { continue }
                v.markerTintColor = parent.vm.alarmedStopIds.contains(s.stop.id) ? .systemYellow : (parent.vm.isStopSelected(s.stop.id) ? .systemOrange : .systemRed)
            }
        }
        ...
        func safeDeconflictAll(_ mapView: MKMapView) {
            ...
            for bus in buses {
                guard let v = mapView.view(for: bus), let nearest = stops.min(by: { CLLocation(latitude: $0.coordinate.latitude, longitude: $0.coordinate.longitude).distance(from: CLLocation(latitude: bus.coordinate.latitude, longitude: bus.coordinate.longitude)) < CLLocation(latitude: $1.coordinate.latitude, longitude: $1.coordinate.longitude).distance(from: CLLocation(latitude: bus.coordinate.latitude, longitude: bus.coordinate.longitude)) }) else { continue }
                let dist = CLLocation(latitude: nearest.coordinate.latitude, longitude: nearest.coordinate.longitude).distance(from: CLLocation(latitude: bus.coordinate.latitude, longitude: bus.coordinate.longitude))
                if dist <= 8.0 {
                    ...
                    (v as MKAnnotationView).centerOffset = CGPoint(x: (CGFloat(dx/mag) * 6.0), y: -10 + (CGFloat(-dy/mag) * 6.0))
                } else {
                    (v as MKAnnotationView).centerOffset = CGPoint(x: 0, y: -10)
                }
            }
        }
        ...
        func mapView(_ mapView: MKMapView, rendererFor overlay: MKOverlay) -> MKOverlayRenderer {
            if let line = overlay as? MKPolyline {
                ...
                r.strokeColor = (line.title == "busTrail") ? .systemOrange : ((line.title == "futureRoute") ? .systemRed : .systemGray)
                r.lineWidth = (line.title == "futureRoute" || line.title == "busTrail") ? 4 : 3
...
struct BusMapScreen: View {
    @StateObject private var vm = MapVM.shared // 싱글톤 인스턴스 사용 권장
    ...
    var body: some View {
        VStack(spacing: 0) {
            // MARK: - 1. 상단 상세 맵 (Magnified Detail)
            ZStack(alignment: .bottom) {
                ClusteredMapView(
                    vm: vm,
                    recenterRequest: .constant(false),
                    mode: .detail
                )
            ...
            // MARK: - 2. 중앙 ROI 정보 바 (상하단 구분 및 개수 표시)
            ZStack {
                Color(UIColor.secondarySystemBackground).frame(height: 30)
                HStack(spacing: 15) {
                    HStack(spacing: 4) {
                        Image(systemName: "bus.fill")
                        Text("\(vm.busCountInROI)")
                    }
                    Divider().frame(height: 12)
                    HStack(spacing: 4) {
                        Image(systemName: "mappin.and.ellipse")
                        Text("\(vm.stopCountInROI)")
                    }
                }
                    ...
                    onAskAlarmForStop: { stop in
                        Task {
                            ...
                            LocalAlertCenter.shared.scheduleOneTime(
                                stop: stop, routes: nil, at: fire, etaSummary: summary
                            )
        ...
        .sheet(isPresented: $showAlarmSheet) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                if let s = alarmTargetStop { Text("정류장: \(s.name)").font(.subheadline) }
                    ...
                    Button("저장") {
                        Task {
                            ...
                            if let m = Int(repeatMinutesText), m >= 1 {
                                LocalAlertCenter.shared.scheduleRepeating(stop: s, routes: nil, every: m, etaSummary: summary)
                            } else {
                                LocalAlertCenter.shared.scheduleOneTime(stop: s, routes: nil, at: alarmDate, etaSummary: summary)
                            }
...
final class RealisticBusView: MKAnnotationView {
    private let label = UILabel()
    private let busIcon = UIImageView()
    private let container = UIView()

    override init(annotation: MKAnnotation?, reuseIdentifier: String?) {
        super.init(annotation: annotation, reuseIdentifier: reuseIdentifier)
        self.frame = CGRect(x: 0, y: 0, width: 80, height: 75)
        ...
        // 2. 실제 버스 모양 아이콘 (2층 버스 스타일로 더 리얼하게)
        busIcon.image = UIImage(systemName: "bus.doubledecker.fill")
        busIcon.tintColor = .systemBlue
        busIcon.contentMode = .scaleAspectFit
        busIcon.frame = CGRect(x: 15, y: 25, width: 50, height: 50)
        
        // 아이콘에 흰색 테두리 효과 (지도 대비)
        busIcon.layer.shadowColor = UIColor.white.cgColor
        busIcon.layer.shadowRadius = 5
        busIcon.layer.shadowOpacity = 1
        busIcon.layer.shadowOffset = .zero
        container.addSubview(busIcon)

        // 3. 버스 번호판 (전광판 느낌)
        label.font = .systemFont(ofSize: 12, weight: .black)
        label.textColor = .yellow // 전광판처럼 노란색 글씨
        label.backgroundColor = .black // 검은색 배경
        ...
        // 가로 길이를 넉넉히 하고 자동 폰트 조절
        label.frame = CGRect(x: 5, y: 5, width: 70, height: 22)
    ...
    override func prepareForDisplay() {
        super.prepareForDisplay()
        if let b = annotation as? BusAnnotation {
            label.text = b.routeNo
        }
    }
    required init?(coder: NSCoder) { fatalError() }
}

// --- 상단 상세 맵: 표지판 모양 정류장 뷰 ---
// --- 상단 상세 맵: 컴팩트한 표지판 모양 정류장 뷰 ---
final class RealisticStopView: MKAnnotationView {
    private let label = UILabel()
    private let signIcon = UIImageView()

    override init(annotation: MKAnnotation?, reuseIdentifier: String?) {
        super.init(annotation: annotation, reuseIdentifier: reuseIdentifier)
        
        // 1. 전체 프레임 축소 (너비 130 -> 90, 높이 70 -> 55)
        self.frame = CGRect(x: 0, y: 0, width: 90, height: 55)
        self.centerOffset = CGPoint(x: 0, y: -25) // 중심점 조정

        // 2. 정류장 아이콘 (심플하고 작게)
        signIcon.image = UIImage(systemName: "signpost.right.fill")
        signIcon.tintColor = .systemRed
        signIcon.contentMode = .scaleAspectFit
        // 아이콘을 중앙 상단에 배치
        signIcon.frame = CGRect(x: 35, y: 0, width: 20, height: 22)
        
        // 아이콘 그림자 효과 (작게)
        signIcon.layer.shadowColor = UIColor.black.cgColor
        signIcon.layer.shadowRadius = 1
        signIcon.layer.shadowOpacity = 0.2
        signIcon.layer.shadowOffset = CGSize(width: 0.5, height: 0.5)
        addSubview(signIcon)

        // 3. 정류장 이름 라벨 (컴팩트한 레드 박스)
        label.font = .systemFont(ofSize: 9, weight: .bold) // 폰트 크기 살짝 축소
        ...
        // 라벨 위치 조정 (아이콘 바로 아래에 밀착)
        label.frame = CGRect(x: 0, y: 22, width: 90, height: 26)
    ...
    var body: some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(alignment: .center) {
                ...
                // ✅ 패널 닫기 버튼: 모든 추적 상태 초기화
                Button {
                    withAnimation(.easeInOut(duration: 0.2)) {
                        vm.followBusId = nil    // 패널 닫기
                        vm.stopTrail()          // 지나온 주황색 선 제거
                        vm.clearFutureRoute()   // 앞으로 갈 빨간색 선 제거
                    }
                } label: {
...
// REPLACE: StopETAInfoPanel 전체 교체
struct StopETAInfoPanel: View {
    @ObservedObject var vm: MapVM
    var onTapAlarm: () -> Void

    // 스크롤 높이 한도 (화면 높이의 35% 또는 220 중 큰 값으로)
    private var maxListHeight: CGFloat {
        max(UIScreen.main.bounds.height * 0.35, 220)
    }
        ...
        if let s = vm.focusStop {
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack {
                    ...
                    Spacer()
                    Button {
                        withAnimation { vm.setFocusStop(nil) }
                    } label: {
                        Image(systemName: "xmark")
                    }
                    .buttonStyle(.plain)
                ...
                Group {
                    if vm.focusStopLoading {
                        ...
                        ScrollView(.vertical, showsIndicators: true) {
                            etaList()      // 리스트 자체는 그대로 사용
                                .padding(.vertical, 2)
                        }
                        .frame(maxHeight: maxListHeight)   // ⬅️ 스크롤 높이 제한
                    }
                }
